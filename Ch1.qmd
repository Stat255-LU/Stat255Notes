...
A. Calculate summary statistics in R.\
...
We'll work with data on houses that sold in King County, WA, (home of Seattle) between 2014 and 2015. The data used in this section are a subset of the data available in the \[Kaggle "House Sales in King County, USA"\] (https://www.kaggle.com/datasets/harlfoxem/housesalesprediction) dataset.
...
Let's create a smaller version of the dataset, with only the columns ID, price, sqft_living, and waterfront. We'll call this `Houses_4var.`
...
We see that waterfront houses tend to be more expensive, on average, and also have more variability (higher standard deviation) in price than non-waterfront houses.
...
Density plots show the distribution for the quantitative variable, price. Scores can be compared across categories, like whether or not the house is on a waterfront.
...
You can make the plot horizontal by adding `+ coord_flip()`. You can turn the axis text vertical by adding `theme(axis.text.x = element_text(angle = 90))`.
...
We see that the majority of houses are in average or below condition. Some are in good condition, and a few are in very good condition.
...
We see that a slightly higher proportion of waterfront houses are in good or very good condition than non-waterfront houses.
...
The `Coasters` dataset contains information on roller coasters around the world. More information about the variables are available [here](https://math.montana.edu/courses/s216/data/rollercoaster_descrip.html).
...
The following histograms and boxplots provide information on the average monthly temperatures, in degrees Celsius, for the 24 months in 2014-15, in the cities of Moscow (Russia), Melbourne (Australia), and San Francisco (United States). Use the graphs to answer the following questions.

```{r, echo=FALSE, fig.width=7, fig.height=4, fig.align='center', message=FALSE, warning=FALSE}
data("CityTemps2e")
Temps <- CityTemps2e %>% gather(key = City, value = Temperature, Moscow:San.Francisco )
```
...
The American Cancer Society and National Cancer Institute distribute pamphlets to inform patients with cancer about the nature of the disease and possible treatments. It is important that pamphlets be written at a reading level that patients will be able to understand.
...
How will mean household size estimates compare for the two approaches? Will one approach yield a larger estimate of household size? Why or why not?
