...
FloridaLakes <- FloridaLakes |> select(ID, Lake, pH, Mercury, Location)
FloridaLakes <- data.frame(FloridaLakes)
```
...
```{r}
Bootstrap1 <- Bootstrap_Lakes |> filter(replicate == 1) |> arrange(ID)
Bootstrap1
```
...
```{r}
Bootstrap2 <- Bootstrap_Lakes |> filter(replicate == 2) |> arrange(ID)
Bootstrap2 
```
...
```{r}
Lakes_Bootstrap_Mean <- FloridaLakes |>
...
```{r}
visualize(Lakes_Bootstrap_Mean) + xlab("Sample Mean") 
```
...
```{r}
SE_mean <- sd(Lakes_Bootstrap_Mean$stat)
SE_mean
```
...
```{r}
Lakes_Mean_CI <- CI <-Lakes_Bootstrap_Mean |> get_confidence_interval(level = 0.95, type = "percentile")

Lakes_Mean_CI
```


We are 95% confident that the average mercury level among all Florida lakes is between `r round(Lakes_Mean_CI$lower_ci, 2)` and `r round(Lakes_Mean_CI$upper_ci, 2)` parts per million.   

The 95% confidence interval is shown by the gold bar on the graph of the bootstrap distribution below.


```{r}
visualize(Lakes_Bootstrap_Mean) +
  shade_confidence_interval(CI) + xlab("Sample Mean")
```
...
```{r}
Lakes_Mean_CI_se <- CI <-Lakes_Bootstrap_Mean |> 
  get_confidence_interval(level = 0.95, type = "se",
                          point_estimate = mean(FloridaLakes$Mercury))
...
```{r}
Lakes_Bootstrap_Prop <- FloridaLakes |>
                          specify(response = High_Mercury, success = "TRUE") |>
                          generate(reps = 10000, type = "bootstrap") |>
                          calculate(stat = "prop")
```
...
```{r}
visualize(Lakes_Bootstrap_Prop, bins=17) + xlab("Sample Proportion") 
```

```{r}
Lakes_Prop_CI <- CI <-Lakes_Bootstrap_Prop |> get_confidence_interval(level = 0.95, type = "percentile")

Lakes_Prop_CI
```


We are 95% confident that the average mercury level among all Florida lakes is between `r round(Lakes_Prop_CI$lower_ci, 2)` and `r round(Lakes_Prop_CI$upper_ci, 2)` parts per million.   

```{r}
visualize(Lakes_Bootstrap_Prop, bins=17) +
  shade_confidence_interval(CI) + xlab("Sample Proportion")
```
...
```{r}
SE_p_hat <- sd(Lakes_Bootstrap_Prop$stat)
SE_p_hat
```
...
```{r}
Lakes_Prop_CI_se <- CI <-Lakes_Bootstrap_Mean |> 
  get_confidence_interval(level = 0.95, type = "se",
                          point_estimate = prop(FloridaLakes$High_Mercury==TRUE))
...
We've seen how to use boostrapping to obtain confidence intervals for means and proportions. We can use the same process to obtain confidence intervals for other statistics, such as regression coefficients $b_0$, $b_1$, etc.  
...
Thus, we can obtain a confidence interval for the difference in average mercury levels by fitting a regression model to each of our bootstrap samples and recording the value of the sample statistic $b_1$, which represents the difference in means between Northern and Southern Florida.   
...
We use the `specify` function to tell R to fit a model with Mercury as the response variable and Location as the explanatory variable, and the `generate` function to generate 10000 bootstrap samples from the model. `strata=Location` tells R to choose the same number of Northern and Southern Lakes in the original sample in each bootstrap sample, matching the numbers from the original sample.    
 
```{r}
#Take 10,000 bootstrap samples. 
BootstrapSamples <- FloridaLakes |>
  generate(reps = 10000, type = "bootstrap")
```
...
```{r}
# Filter to the first bootstrap sample
BootstrapSample1 <- BootstrapSamples %>%
  filter(replicate == 1)

BootstrapSample1 |> arrange(ID)
```

```{r}
Bootstrap1_BP <- ggplot(data=BootstrapSample1, aes(x=Location, y=Mercury, fill=Location)) + 
  geom_boxplot() +   geom_jitter() + ggtitle("Mercury Levels in Florida Lakes") + 
  xlab("Location") + ylab("Mercury Level") + theme(axis.text.x = element_text(angle = 90)) + coord_flip()
Bootstrap1_BP
```
...
```{r}
Bootstrap1_BP <- BootstrapSample1 %>% group_by(Location) %>% summarize(MeanHg=mean(Mercury), 
                                                  StDevHg=sd(Mercury), 
                                                  N=n())
Bootstrap1_BP
```
...
```{r}
# Fit a linear model
Mb1 <- lm(Mercury ~ Location, data = BootstrapSample1)
Mb1
```
...
```{r}
# Filter to the first bootstrap sample
BootstrapSample2 <- BootstrapSamples %>%
  filter(replicate == 2)
...
```{r}
Bootstrap2_BP <- ggplot(data=BootstrapSample1, aes(x=Location, y=Mercury, fill=Location)) + 
  geom_boxplot() +   geom_jitter() + ggtitle("Mercury Levels in Florida Lakes") + 
  xlab("Location") + ylab("Mercury Level") + theme(axis.text.x = element_text(angle = 90)) + coord_flip()
Bootstrap2_BP
```
...
```{r}
Bootstrap2_BP <- BootstrapSample2 %>% group_by(Location) %>% summarize(MeanHg=mean(Mercury), 
                                                  StDevHg=sd(Mercury), 
                                                  N=n())
Bootstrap2_BP
```
...
```{r}
# Fit a linear model
Mb2 <- lm(Mercury ~ Location, data = BootstrapSample2)
Mb2
```
...
```{r}
Lakes_lm_boot <- do(1000) * predict(lm(Mercury ~ Location, data = resample(FloridaLakes)), newdata=data.frame(Location="S"), interval = "confidence", level=0.95)

confint(Lakes_lm_boot, parm = "fit", method = "stderr")
```
...
`r LakesPermTable[1,2] - LakesPermTable[2,2]`
