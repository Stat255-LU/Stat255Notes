...
Houses <- Houses |> mutate(waterfront = as.factor(waterfront))
```

## Fitting Models to Data

### King County, WA Houses

**Research Question: How much does being on the waterfront add to the value of a house?**

King County Washington, home of Seattle, contains scenic waterfront properties along Elliott Bay, Washington Lake, and Lake Sammamish. In this section, we'll how being located on the waterfront affects the price of a house.

We work with a sample of 200 houses that sold between 2014 and 2015 in King County, WA. A total of 25 houses were located on the waterfront, while the other 175 houses were not on the waterfront.

The data used in this section are a subset of the data available in the \[Kaggle "House Sales in King County, USA"\] (https://www.kaggle.com/datasets/harlfoxem/housesalesprediction) dataset.

### Terminology

In this section, we'll use statistical models to predict the prices of houses in King County, WA, using information about whether or not they lie on the waterfront.
...
We'll predict the price of a house based on whether or not it is on the waterfront.

The boxplot shows the distribution of prices for waterfront and non-waterfront houses. The red dots indicate the mean.
...
The coefficient $b_1$ represents the expected (or average) difference in response between a category and the "baseline" category.

- On average, a house that is not on the waterfront is expected to cost `r M_House_price_wf_b0` thousand dollars.

- On average a house that is on the waterfront is expected to cost `r M_House_price_wf_b1 |> round(2)` thousand (or `r (M_House_price_wf_b1/1000)|> round(2)` million) dollars more than a house that is not on the waterfront.
...
$$\widehat{\text{Price}} = `r M_House_price_wf_b0` + `r M_House_price_wf_b1`\times 1 = `r round(M_House_price_wf_b0 + M_House_price_wf_b1,2)`{ \text{ thousand (or `r round((M_House_price_wf_b0 + M_House_price_wf_b1)/1000,2)` million)}}$$

The predicted price of a house not on the waterfront is:

$$\widehat{\text{Price}} = `r M_House_price_wf_b0` + `r M_House_price_wf_b1`\times 0 = `r round(M_House_price_wf_b0,2) `{ \text{ thousand}}$$
...
It's possible that houses might be different from houses not on the waterfront in other ways as well. For example, waterfront houses might be bigger than non-waterfront houses. To account for this, we can add the number of square feet in the house to our model. This ensures we are comparing houses of the same size.

A model with two or more explanatory variables is called a **multiple regression model**.

The model equation is

$$\widehat{\text{Price}} = b_0 + b_1\times\text{Waterfront}  + b_2\times\text{Sq. Ft}$$

For a house not on the waterfront, the $\text{Waterfront} = 0$ so the model equation is:

$$\widehat{\text{Price}} = b_0  + b_2\text{Sq. Ft} $$

For a house on the waterfront, $\text{Waterfront} = 1$, so the model equation is:

$$\widehat{\text{Price}} = (b_0 + b_1) + b_2\times\text{Sq. Ft} $$

These are both linear equations with slope $b_2$. The y-intercept, however, is different ($b_0$ for houses not on the waterfront, and $b_0 + b_1$ for houses on the waterfront). Thus, the model assumes that price increases at the same rate, with respect to square feet, regardless of whether or not it is on the waterfront, but allows the predicted price for a waterfront house to differ from a non-waterfront house of the same size.
...
Plot_House_price_wf_sqft <- ggplot(data=Houses, aes(x=sqft_living, y=price, color=waterfront)) + 
  geom_point() + 
  theme_bw() + 
  geom_parallel_slopes(se=FALSE) 

Plot_House_price_wf_sqft
```

We fit the model in R.

```{r}
M_House_price_wf_sqft <- lm(data=Houses, price~waterfront + sqft_living)
M_House_price_wf_sqft
```

```{r, echo=FALSE}
M_House_price_wf_sqft_b0 <- round(M_House_price_wf_sqft$coefficients[1],2)
M_House_price_wf_sqft_b1 <- round(M_House_price_wf_sqft$coefficients[2],2)
M_House_price_wf_sqft_b2 <- round(M_House_price_wf_sqft$coefficients[3],2)
```

The model equation is

$$\widehat{\text{Price}} = `r M_House_price_wf_sqft_b0` + `r M_House_price_wf_sqft_b1`\times\text{Waterfront} + `r M_House_price_wf_sqft_b2`\times\text{Sq. Ft}$$
...
For a categorical variable, we interpret the coefficient $b_j$ as the expected difference in the response variable between that category and the "baseline" category, assuming all other explanatory variables are the same.

For a quantitative variable, we interpret the coefficient $b_j$ as the expected change in the response variable for each one-unit increase of the explanatory variable, assuming all other explanatory variables are the same.

- On average, a house that is not on the waterfront with 0 square feet is expected to cost `r M_House_price_wf_sqft_b0` thousand dollars. This is not a sensible interpretation, since there are no houses with 0 square feet.\

- On average, a house on the waterfront is expected to cost `r M_House_price_wf_sqft_b1` thousand dollars more than a house that is not on the waterfront, assuming square footage is the same.

- For each 1-square foot increase in size, the price of a house is expected to increase by `r M_House_price_wf_sqft_b2` thousand (or `r M_House_price_wf_sqft_b2*1000`) dollars, assuming waterfront status is the same. Equivalently, for each 100-square foot increase in size, the price of a house is expected to increase by `r M_House_price_wf_sqft_b2*100` thousand dollars, assuming waterfront status is the same.

**Question for thought:** Notice that the estimated difference in price between waterfront and non-waterfront houses, after accounting for size, is `r M_House_price_wf_sqft_b1`, which is less than the estimated difference when we did not account for size, which was `r M_House_price_wf_b1`. Why is this? Hint: look at the scatterplot displaying house price, size, and waterfront status.

**Prediction**

The predicted price of a 2,000 square foot house on the waterfront is:

$$\widehat{\text{Price}} = `r M_House_price_wf_sqft_b0` + `r M_House_price_wf_sqft_b1`\times1 + `r M_House_price_wf_sqft_b2`\times2000 = `r round(M_House_price_wf_sqft$coefficients[1] +  M_House_price_wf_sqft$coefficients[2] + M_House_price_wf_sqft$coefficients[3]*2000 , 2)`{ \text{ thousand (or `r round((M_House_price_wf_sqft$coefficients[1] +  M_House_price_wf_sqft$coefficients[2] + M_House_price_wf_sqft$coefficients[3]*2000)/1000,2)` million)}}$$

The predicted price of a 2,000 square foot house not on the waterfront is:

$$\widehat{\text{Price}} = `r M_House_price_wf_sqft_b0` + `r M_House_price_wf_sqft_b2`\times2000 = `r round( M_House_price_wf_sqft$coefficients[1] +  M_House_price_wf_sqft$coefficients[3]*2000, 2)`{ \text{ thousand}}$$

Calculations in R:

```{r}
predict(M_House_price_wf_sqft, newdata=data.frame(waterfront="Yes", sqft_living=2000))
```

```{r}
predict(M_House_price_wf_sqft, newdata=data.frame(waterfront="No", sqft_living=2000))
```

We should only try to make predictions on houses within the range of the observed data. Since the largest house in the dataset is 8,000 square feet we should not try to predict the price of house with 10,000 square feet. Making predictions outside the range of the data is called **extrapolating** and is dangerous. Since trends we observe may not continue outside the range of the data we observe, it can lead to nonsensical results.

**Conclusion:** The data show that waterfront houses tend to be substantially more expensive than houses that are not on the waterfront. Based on the 200 houses in our sample, we estimated that waterfront houses cost `r M_House_price_wf_b1` thousand (i.e `r M_House_price_wf_b1/1000`) million dollars more than non-waterfront houses, on average. Comparing houses of the same size, the difference is smaller, but still substantial, as we estimated that a waterfront house is estimated to cost `r M_House_price_wf_sqft_b1` thousand dollars more than a non-waterfront house with the same number of square feet.

## Variability Explained by a Model

### Measuring Variability Explained

**Central Question: How do we compare models and measure how well they're fitting the data?**

We've seen a model for predicting house price based only on whether or not it is on the waterfront, and another that also considered the size of the house. We want to be able to measure how much information we gain when addition variable(s) are added to a model. To do that, we'll compare each model to a model that contains no explanatory variables (i.e. no information about the houses other than price). This will tell us how much the information provided by waterfront status and square feet help us in predicting price.
...
We'll calculate the residuals for three houses in the dataset, shown below.

```{r}
Houses$ID <- 1:200

Three_Houses <- Houses %>% select(ID, price, waterfront, sqft_living) %>% slice(c(3, 37, 50))
kable(Three_Houses)
```
...
$$\widehat{\text{Price}_3} = `r M_House_price_wf_sqft_b0` + `r M_House_price_wf_sqft_b1`\times `r as.numeric(Houses$waterfront[3]=="Yes")` + `r M_House_price_wf_sqft_b2`\times `r Houses$sqft_living[3]` = `r round(M_House_price_wf_sqft$fit[3], 1)` \text{ thousand dollars}$$

$$\widehat{\text{Price}_{37}} = `r M_House_price_wf_sqft_b0` + `r M_House_price_wf_sqft_b1`\times `r as.numeric(Houses$waterfront[37]=="Yes")` + `r M_House_price_wf_sqft_b2`\times `r Houses$sqft_living[37]` = `r round(M_House_price_wf_sqft$fit[37], 1)` \text{ thousand dollars}$$

$$\widehat{\text{Price}_{50}} = `r M_House_price_wf_sqft_b0` + `r M_House_price_wf_sqft_b1`\times `r as.numeric(Houses$waterfront[50]=="Yes")` + `r M_House_price_wf_sqft_b2`\times `r Houses$sqft_living[50]` = `r round(M_House_price_wf_sqft$fit[50], 1)` \text{ thousand dollars}$$

To calculate the residuals, we subtract the predicted price from the actual price.

$$r_{3} = y_1-\hat{y}_{3} = `r Houses$price[3]` - `r round(M_House_price_wf_sqft$fit[3],1)` = `r round(M_House_price_wf_sqft$resid[3],0)`  \text{ thousand dollars}$$

$$r_{37} = y_2-\hat{y}_{37} = `r Houses$price[37]` - `r round(M_House_price_wf_sqft$fit[37],1)` = `r round(M_House_price_wf_sqft$resid[37],0)` \text{ thousand dollars}$$

$$r_{50} = y_3-\hat{y}_{50} = `r Houses$price[50]` - `r round(M_House_price_wf_sqft$fit[50],1)` = `r round(M_House_price_wf_sqft$resid[50],0)` \text{ thousand dollars}$$
...
The predicted values and residuals from a model can be calculated automatically in R. The predicted values and residuals for the 3 houses are shown below.

```{r}
Predicted <- predict(M_House_price_wf_sqft)
Predicted[3]
Predicted[37]
Predicted[50]
```
...
```{r}
Residual <- M_House_price_wf_sqft$residuals
Residual[3]
Residual[37]
Residual[50]
...
The actual and predicted values for all 200 houses in the dataset are shown below. Dots represent actual values, the regression lines represents the predicted values, and each vertical displays the residuals.

```{r,  fig.height=5, fig.width=12}
P_Resid_M_House_price_wf_sqft <- ggplot(data=Houses, aes(x=sqft_living, y=price, color=waterfront)) + geom_point()+ geom_parallel_slopes(se=FALSE) + 
              geom_segment(aes(xend = sqft_living, yend = M_House_price_wf_sqft$fitted.values), color="red")+theme_bw()  + theme(legend.position = "none") + ylim(c(0,5000))
P_Resid_M_House_price_wf_sqft
```
...
### Measuring Variability Explained by a Model

To measure the amount of variability in the response variable explained by a model, we calculate the sum of the squared residuals and compare it to the sum of squared residuals for a model with no explanatory variables (i.e. no information other than the response variable). In the house price dataset, this tells us how much better we do at explaining variability in price using information about waterfront status and size than we would have done if we didn't have this information and knew only the prices of the houses.

In a model with no explanatory variables, the best prediction we can make is to simply predict the price of every house to be equal to the mean price of the houses in the dataset, which is `r mean(Houses$price) |> round(0)` thousand dollars.

The graph on the left shows residuals for the model using waterfront status and size, while the graph on the right shows residuals for the model using no explanatory variables and simply predicting every house to cost `r mean(Houses$price) |> round(0)` thousand dollars.

```{r, echo=FALSE,fig.height=5, fig.width=12}
grid.arrange(P_Resid_M_House_price_wf_sqft+ ggtitle("Residuals for Model with  waterfront status and Sq.Ft."), P_Resid_M_House_price_0 + ggtitle("Residuals for model with no Exp. Vars"), ncol=2)
```
...
We measure how well the model fits the observed data using the sum of the squared residuals. We have to square the residuals because if we just added up the residuals themselves, some would be positive and others negative and they would, in fact add to zero. By squaring, we ensure that each contribution is positive and larger square residuals represent bigger differences between observed and predicted values. This could also be achieved by taking the sum of the absolute values of the residuals, but there are certain advantages to using squared residuals and that is what is commonly done.
...
$$\begin{aligned}
\text{SSR} & = \displaystyle\sum_{i=1}^{n}(y_i-\hat{y}_i)^2.  \\
 & = (`r Houses$price[3]` - `r round(M_House_price_wf_sqft$fit[3],1)`)^2 + (`r Houses$price[37]` - `r round(M_House_price_wf_sqft$fit[37],1)`)^2 + (`r Houses$price[50]` - `r round(M_House_price_wf_sqft$fit[50],1)`)^2 + \ldots
\end{aligned}$$
...
```{r}
SSR_M_House_price_wf_sqft <- sum(M_House_price_wf_sqft$residuals^2)
SSR_M_House_price_wf_sqft
```

We compare SSR for our model to the sum of the squared differences we would get if we based our predictions on a model with no explanatory variables and used the overall mean of the response variable as the prediction for every case. This quantity is called the **total sum of squares (SST)**.

$$\text{SST} = \displaystyle\sum_{i=1}^n (y_i - \bar{y})^2$$

```{r, echo=FALSE}
meanprice <- mean(Houses$price) |> round(0)  #calculate mean price
```

$$\begin{aligned}
\text{SST} & = \displaystyle\sum_{i=1}^{200} (y_i - \bar{y})^2 \\
& = (`r Houses$price[3]`- `r meanprice`)^2 + (`r Houses$price[37]`- `r meanprice`)^2 + (`r Houses$price[50]`- `r meanprice`)^2 + \ldots 
\end{aligned}$$
...
By itself, the size of SST and SSR do not have much meaning, since they depend on the size and scale of the variable being measured. An SST value that is very large in one context might be very small in another. We can, however, compare SST and SSR to see how much information we gain by using the variables in the model. SST will always be larger than (or equal to) SSR. The smaller SSR is, compared to SST, the more the variables in the model have helped reduce the size of the residuals by explaining variability in the response variable.

We calculate the difference between SST and SSR and call this quantity the **sum of squares model (SSM)**. This represents the amount of variability in the response variable that is explained by the model.

$$SSM = SST-SSR = `r SST|>round(0)` - `r SSR_M_House_price_wf_sqft|>round(0)` = `r (SST_M_House_price - SSR_M_House_price_wf_sqft)|>round(0)`.$$

It can be shown that $\text{SSM}=\displaystyle\sum_{i=1}^n(\hat{y}_i-\bar{y})^2$.

Since SSR, SST, and SSM depend on the units of the response variable, there is no universal way to determine what constitutes large or small values for these quantities. We can, however, develop a universal standard for comparison by dividing SSM by SST. This gives a ratio between zero and one, which represents the proportion of total variability in the response variable explained by the model. This quantity is called the **coefficient of determination**, denoted $R^2$.

$$R^2=\frac{SSM}{SST}= \frac{SST-SSR}{SST}$$

$$R^2 = \frac{`r (SST_M_House_price - SSR_M_House_price_wf_sqft)|>round(0)`}{`r SST_M_House_price|>round(0)`}=`r summary(M_House_price_wf_sqft)$r.squared|>round(2)`.$$

Approximately `r (summary(M_House_price_wf_sqft)$r.squared*100)|>round(1)`% of the total variability in sale price is explained by the model using square feet and waterfront status as explanatory variables.

```{r, echo=FALSE}
slices <- c( round(SSR_M_House_price_wf_sqft,0), round(SST-SSR_M_House_price_wf_sqft,0))
lbls <- c("SSR", "SSM")
...
```{r}
summary(M_House_price_wf_sqft)$r.squared |> round(4)
```
...
Notice that after accounting for waterfront status, the differences between observed and predicted values are bigger than they were in the model that accounted for square feet, in addition to waterfront status.

The predicted prices for the three houses we've seen before are:

```{r}
Three_Houses
```

$$\widehat{\text{Price}_3} = `r M_House_price_wf_b0` + `r M_House_price_wf_b1`\times `r as.numeric(Houses$waterfront[3]=="Yes")` = `r round(M_House_price_wf$fit[3], 1)` \text{ thousand dollars}$$

$$\widehat{\text{Price}_{37}} = `r M_House_price_wf_b0` + `r M_House_price_wf_b1`\times `r as.numeric(Houses$waterfront[37]=="Yes")` = `r round(M_House_price_wf$fit[37], 1)` \text{ thousand dollars}$$

$$\widehat{\text{Price}_{50}} = `r M_House_price_wf_b0` + `r M_House_price_wf_b1`\times `r as.numeric(Houses$waterfront[50]=="Yes")` = `r round(M_House_price_wf$fit[50], 1)` \text{ thousand dollars}$$

To calculate the residuals, we subtract the predicted price from the actual price.

$$r_1 = y_1-\hat{y}_{3} = `r Houses$price[3]` - `r round(M_House_price_wf$fit[3],1)` = `r round(M_House_price_wf$resid[3],0)`  \text{ thousand dollars}$$

$$r_2 = y_2-\hat{y}_{37} = `r Houses$price[37]` - `r round(M_House_price_wf$fit[37],1)` = `r round(M_House_price_wf$resid[37],0)` \text{ thousand dollars}$$

$$r_3 = y_3-\hat{y}_{50} = `r Houses$price[50]` - `r round(M_House_price_wf$fit[50],1)` = `r round(M_House_price_wf$resid[50],0)` \text{ thousand dollars}$$
...
Approximately `r (summary(M_House_price_wf)$r.squared*100) |> round(0)`% of the total variability in sale price is explained by the model using waterfront status as the explanatory variable.
...
```{r}
summary(M_House_price_wf)$r.squared |> round(4)
```

### Variability Explained by Sq. Ft. Model

We've seen a model using both square feet and waterfront status, and also one using only waterfront status. Now, we'll consider one using only square feet.
...
In this model, we only have a single regression line relating price to square feet, rather than two lines, as we saw when we accounted for waterfront status.
...
$$\widehat{\text{Price}_{37}} = `r M_House_price_sqft_b0` + `r M_House_price_sqft_b1`\times `r Houses$sqft_living[37]` = `r round(M_House_price_sqft$fit[37], 1)` \text{ thousand dollars}$$

$$\widehat{\text{Price}_{50}} = `r M_House_price_sqft_b0` + `r M_House_price_sqft_b1`\times `r Houses$sqft_living[50]` = `r round(M_House_price_sqft$fit[50], 1)` \text{ thousand dollars}$$

To calculate the residuals, we subtract the predicted price from the actual price.

$$r_1 = y_1-\hat{y}_{3} = `r Houses$price[3]` - `r round(M_House_price_sqft$fit[3],1)` = `r round(M_House_price_sqft$resid[3],0)`  \text{ thousand dollars}$$

$$r_2 = y_2-\hat{y}_{37} = `r Houses$price[37]` - `r round(M_House_price_sqft$fit[37],1)` = `r round(M_House_price_sqft$resid[37],0)` \text{ thousand dollars}$$

$$r_3 = y_3-\hat{y}_{50} = `r Houses$price[50]` - `r round(M_House_price_sqft$fit[50],1)` = `r round(M_House_price_sqft$resid[50],0)` \text{ thousand dollars}$$

We calculate SSR for the model using square feet as the explanatory variable.

$$\begin{aligned}
\text{SSR} & = \displaystyle\sum_{i=1}^{n}(y_i-\hat{y}_i)^2.  \\
 & = (`r Houses$price[3]` - `r round(M_House_price_sqft$fit[3],1)`)^2 + (`r Houses$price[37]` - `r round(M_House_price_sqft$fit[37],1)`)^2 + (`r Houses$price[50]` - `r round(M_House_price_sqft$fit[50],1)`)^2 + \ldots
\end{aligned}$$
...
$$R^2 = \frac{`r round(SST_M_House_price - SSR_M_House_price_sqft,0)`}{`r round(SST_M_House_price,0)`}=`r round(summary(M_House_price_sqft)$r.squared,2)`.$$

Approximately `r round(summary(M_House_price_sqft)$r.squared*100, 0)`% of the total variability in sale price is explained by the model using square feet as the explanatory variable.
...
```{r}
summary(M_House_price_sqft)$r.squared |> round(4)
```
...
```{r}
M_House_price_wf_sqft_age <- lm(data=Houses, price ~ sqft_living + waterfront + age)
M_House_price_wf_sqft_age
```

The age coefficient estimate $b_3$ tells us that for each additional year in age, the expected price is expected to increase by `r M_House_price_wf_sqft_age$coefficients[4]|>round(1)` thousand dollars, assuming square feet and waterfront status are held constant. This is not a very large change.
...
```{r}
SSR_M_House_price_wf_sqft_age <- sum((M_House_price_wf_sqft_age$residuals)^2) |>round(1)
SSR_M_House_price_wf_sqft_age
```

SSM:

```{r}
SSM_M_House_price_wf_sqft_age <- SST_M_House_price - SSR_M_House_price_wf_sqft_age |>round(1)
SSM_M_House_price_wf_sqft_age
```

$R^2$

```{r}
summary(M_House_price_wf_sqft_age)$r.squared |> round(4)
```

Recall that the model with square feet and waterfront, but not age, as explanatory variables had an $R^2$ value of `r (summary(M_House_price_wf_sqft)$r.squared)|>round(4)`. Adding age to the model only resulted in a tiny increase in $R^2$, meaning that age only explains a very small percentage of the variability in price that was not explained by size and waterfront status. Thus, it may not be beneficial to add age to the model.

When a new variable is added SSR will never increase and $R^2$ will never decrease. Thus, adding even unimportant variables typically lead to small increases in $R^2$. This does not necessarily mean we should add these variables. Doing so can make the model harder to interpret. It's also true that making a model more complex by adding additional variables makes it more prone to a phenomenon called "overfitting." This means that the model starts modeling random noise, rather than true relationships in the data it was built on (in this case, the 200 houses), causing it to perform worse when predicting new houses than a simpler model with fewer variables would. We'll look more at overfitting later in the course.
...
| 3 | Sq. Ft. and Waterfront | `r round(SSR_M_House_price_wf_sqft,0)` | `r round(SST_M_House_price-SSR_M_House_price_wf_sqft,0)` | `r round((SST_M_House_price-SSR_M_House_price_wf_sqft)/SST_M_House_price,3)` |
| 4 | Sq. Ft., Waterfront, and Age | `r SSR_M_House_price_wf_sqft_age |>round(0)` | `r round(SST_M_House_price-SSR_M_House_price_wf_sqft_age,0)` | `r round((SST_M_House_price-SSR_M_House_price_wf_sqft_age)/SST_M_House_price,3)` |
...
**Conclusion:** We saw that accounting for the size of the house helps explain variability in price, as $R^2$ improves from `r round((SST_M_House_price-SSR_M_House_price_wf)/SST_M_House_price,3)` to `round((SST_M_House_price-SSR_M_House_price_wf_sqft)/SST_M_House_price,3)` when square feet is added to a model that already includes waterfront status. Accounting for age does not appear very important, as $R^2$ only increases a very small amount to `r round((SST_M_House_price-SSR_M_House_price_wf_sqft_age)/SST_M_House_price,3)` when age is added to the model.

\newpage

## ANalysis Of VAriance

**Central Question: Does adding variable(s) to a model help us explain variability in the response variable?**
...
Let's Calculate an ANOVA F-Statistic to compare Models 2 and 3. This will tell us whether adding information about square feet helps explain variability in price, compared to a model using only waterfront status.
...
$$\widehat{\text{Price}}= b_0+ b_1 \times\text{Waterfront}+ b_2\times\text{sqft\_living}$$

$$\begin{aligned}
F &= \frac{\frac{\text{SSR}_{\text{Reduced}}-\text{SSR}_{\text{Full}}}{p-q}}{\frac{\text{SSR}_{\text{Full}}}{n-(p+1)}} \\
&=\frac{\frac{`r round(SSR_M_House_price_wf,2)` - `r round(SSR_M_House_price_wf_sqft,2)`}{2-1}}{\frac{`r round(SSR_M_House_price_wf,2)`}{200-(2+1)}} \\  
& = `r anova(M_House_price_wf_sqft, M_House_price_wf)$F[2] |>round(2)` 
\end{aligned}$$

We can calculate the statistic directly in R, using the `anova` command.

```{r}
anova(M_House_price_wf_sqft, M_House_price_wf)$F[2] |>round(2)
```

**Conclusion** An F-statistic of `r anova(M_House_price_wf_sqft, M_House_price_wf)$F[2] |>round(2)` is quite large and provides strong evidence that adding waterfront status to a model already including square feet helps better explain variability in sale price.
...
We'll calculate an F-statistic to compare Models 3 and 4. This can help us determine whether it is worthwhile to include age in a model that already includes square feet and waterfront status.

Reduced Model: $$\widehat{\text{Price}}= b_0+ b_1 \times\text{Waterfront} + b_2\times\text{sqft\_living}$$

Full Model: $$\widehat{\text{Price}}= b_0+ b_1 \times\text{Waterfront} + b_2\times\text{sqft\_living} + b_3\times\text{Age}$$

$$\begin{aligned}
F &= \frac{\frac{\text{SSR}_{\text{Reduced}}-\text{SSR}_{\text{Full}}}{p-q}}{\frac{\text{SSR}_{\text{Full}}}{n-(p+1)}} \\
&=\frac{\frac{`r round(SSR_M_House_price_wf_sqft,2)` - `r round(SSR_M_House_price_wf_sqft_age,2)`}{3-2}}{\frac{`r round(SSR_M_House_price_wf_sqft,2)`}{200-(3+1)}} \\  
& = `r anova(M_House_price_wf_sqft_age, M_House_price_wf_sqft)$F[2] |>round(2)` 
\end{aligned}$$

We can calculate the statistic directly in R, using the `anova` command.

```{r}
anova(M_House_price_wf_sqft_age, M_House_price_wf_sqft)$F[2] |>round(2)
```

**Conclusion:** Now, the F-statistic is much smaller than we've seen before, indicating that adding age to the model does not help very much in explaining variability in price.
...
In scenario 1, we notice considerable differences between the groups, relative to the amount of variability within groups. In this scenario, knowing the group an observation is in will help us predict the response for that group, so we should account for the groups in our model. We would obtain a large F-statistic when comparing a model that includes group to one that contains only an intercept term.
...
| F Statistic | Large ($F=`r M_df1_F |> round(1)`$) | Small ($F=`r M_df2_F|> round(1)`$) |
...
We'll fit the model in R. The coefficient estimates for $b_0$, and $b_1$ are shown below.

We previously calculated $SSR_{full}$ to be
...
$$\begin{aligned}
F &= \frac{\frac{\text{SSR}_{\text{Reduced}}-\text{SSR}_{\text{Full}}}{p-q}}{\frac{\text{SSR}_{\text{Full}}}{n-(p+1)}} \\
&=\frac{\frac{`r SST_M_House_price |> round(0)` - `r SSR_M_House_price_wf |> round(0)`}{1-0}}{\frac{`r SSR_M_House_price_wf  |> round(0)`}{200-(1+1)}} \\  
...
**Conclusion:** The F-statistic is very large, indicating strong evidence that waterfront status is associated with the price of a house, which is consistent with what we've observed in our plots as well as $R^2$ calculation.
...
Note that in this case, $p=2$, since the full model includes terms for both good and very good conditions, in addition to the "baseline" category of "average or below". For a categorical variable with $g$ groups, there will always be $g-1$ terms in the model equation, one for each category other than the baseline.

The F-statistic is

$$\begin{aligned}
F &= \frac{\frac{\text{SSR}_{\text{Reduced}}-\text{SSR}_{\text{Full}}}{p-q}}{\frac{\text{SSR}_{\text{Full}}}{n-(p+1)}} \\
&=\frac{\frac{`r SST_M_House_price  |> round(0)` - `r SSR_M_House_price_cond  |> round(0)`}{2-0}}{\frac{`r SSR_M_House_price_cond  |> round(0)`}{200-(2+1)}} \\  
...
For comparing a model with explanatory variable $x$ to one with only an intercept term, we use the models:

Reduced Model: $$\widehat{y}= b_0$$

Full Model: $$\widehat{y}= b_0+ b_1 x$$

Since, $p=1$ and $\text{SSR}_{\text{Reduced}}=\text{SST}$, the F-statistic becomes:
...
$$F= \frac{`r summary(M_dfB)$r.squared|> round(4)`}{1 - `r summary(M_dfB)$r.squared|> round(4)`} \times (91-2) = `r summary(M_dfB)$r.squared/(1-summary(M_dfB)$r.squared)*(91-2)|> round(2)`.$$
...
**Central Question: How do we determine the line of best fit? That is, how do we determine the values of estimates** $b_0$, $b_1$, $b_2$, \ldots?
...
In the illustration, SSR is represented by the total area of the squares. The line of best fit is the one that make the area the smallest.
...
The line $\text{Price} = `r M_House_price_sqft_b0` + `r M_House_price_sqft_b1`\times \text{Square Feet}$ is the "line of best fit" in the sense that it minimizes the sum of the squared residuals (SSR). Any other choices for the slope or intercept of the regression line would result in larger SSR than this line.
...
$$\begin{aligned}
\displaystyle\sum_{i=1}^{200}(y_i-\hat{y}_i)^2 & =\displaystyle\sum_{i=1}^n(y_i-(b_0+b_1x_i))^2 \\
& = (267.5-(b_0+b_1(1590)))^2 + (1875-(b_0+b_1(3280)))^2 + (540-(b_0+b_1(2100)))^2 + \ldots
\end{aligned}$$
...
- Consider a model with a single categorical variable (such as waterfront), with G+1 categories, numbered $g=0, 1, 2, \ldots, G$
...
We now drop design from the model in (a) and use only duration as an explanatory variable. The $R^2$ value is shown below.
...
```{r}
sum((Coasters_filtered$Speed - Mean_Speed)^2)
```
...
Consider three models. Model 1 accounts for only the quantitative variable $x$. Model 2 accounts for only the categorical variable, while Model 3 accounts for both the quantitative variable $x$ and the categorical variable.
...
The F-statistic for comparing Model 0 to Model 1 is shown below. Based on the size of this F-statistic, what can we conclude about the variable(s) in these models?
...
Suppose that instead of using these, we used the model with the estimates of $b_0$, $b_1$, and $b_2$ given below.

$$\widehat{\text{Speed}} = 25 + 0.25\times\text{Duration} + 3\times{\text{DesignSitDown}}$$
...
For the fictional test score example in 16(b), write the expression that would need to be minimized to obtain the values of regression coefficients $b_0$ and $b_1$. Substitute values from the data into your expression. (Hint: The only variables in the expression should be $b_0$ and $b_1$).
