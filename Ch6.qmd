...
32. Identify situations where it is appropriate to use a logistic regression model.\
33. Calculate probabilities and odds using logistic regression.\
34. Interpret logistic regression coefficients in context.\
35. Draw conclusions about logistic regression models based on graphical representations of data.
...
y <- exp(x)/ (1+exp(x))
df <- data.frame(x, y)
gf_line(data=df, y~x) + xlab("x") + ylab("y")
```
