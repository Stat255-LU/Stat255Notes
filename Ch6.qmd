...
For every 1-unit increase in $x$ we expect the log odds of "success" to increase by $\beta_1$.
...
```{r, echo=FALSE}
Grad <- read_csv("Grad.csv")
Grad <- Grad %>% mutate(TopTier = rank%in%c(1)) %>% dplyr::select(gre, TopTier, admit)
```
