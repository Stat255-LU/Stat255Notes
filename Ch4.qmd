...
17. State the assumptions of the normal error regression model in context.\   
18. State the hypotheses associated with each p-value in the `lm` summary table and explain what we should conclude from each test.\    
19. Interpret quantities in the R `lm summary output` including standard errors, residual standard error, $R^$, and the F-statistic.    
20. Use standard error formulas to calculate quantities in R model summary output (including estimates, standard errors, t-statistics, $R^2$, and F-statistics), as well as output for associated tests and confidence intervals.   
21. Distinguish between confidence intervals for model parameters, confidence for expected response, and prediction intervals, and identify the sources of variability contributing to each.   
22. Explain the regression effect in context.\  
23. Identify situations where multiple testing error is a concern and explain how to correct for such errors. 
24. Identify situations where a statistically discernible result might not be practically important, or when a large p-value might still be meaningful.  
...
```{r}
CI <- confint(M_Lakes_merc_loc_Bootstrap, parm = c("LocationS"),
              level = 0.95, method = "se")
CI
```

```{r}
M_Lakes_merc_loc_Bootstrap_Plot_b1 + 
...
SE_b1 <- sd(M_Lakes_merc_pH_Bootstrap$pH)
SE_b1
```
...
The standard error and confidence interval obtained via the theory-based formulas are again very close to those obtained via bootstrapping.  
...
```{r}
sum(M_Lakes_merc_0$residuals^2)
```
...
```{r}
confint(M_Lakes_merc_0, level=0.95)
```
...
Looking at the coefficients table, we've already seen where the first two columns, `Estimate` and `Std. Error` come from. **The last two columns, labeled `t value` and "Pr(\>\|t\|)" represent the t-statistic and p-value for the t-test associated with the null hypothesis that the regression parameter on that line is zero. (i.e.** $\beta_j=0$).
...
The p-value from the permutation test is similar to the one from the t-test. Both lead us to the conclusion that there is evidence of a difference in mean mercury level between lakes in Northern Florida and Southern Florida.   
...
Since there are no lakes with pH level 0, this is not a meaningful test. Although a p-value is provided in the R-output, it is not meaningful and we should not try to draw conclusions from it.   
...
$$
b_0 + b_1x^* \pm t^* s\sqrt{\left(\frac{1}{n}+ \frac{(x^*-\bar{x})^2}{\displaystyle\sum_{i=1}^n(x_i-\bar{x})^2}\right) + 1} 
$$
...
2.  Calculate an interval that we are 95% confident contains the mercury concentration for an individual lake in Northern Florida. Do the same for a lake in Southern Florida.
...
### Breakfast Cereals and Sex of Babies

The title of a [2008 article by New Scientist](https://www.newscientist.com/article/dn13754-breakfast-cereals-boost-chances-of-conceiving-boys/) makes a surprising claim: "Breakfast cereals boost chances of conceiving boys." Although the article lacks details on the underlying study, researchers, in fact, tracked the eating habits of women who were attempting to become pregnant. They tracked 133 different food items, and tested whether there was a difference in the proportion of baby boys conceived between women who ate the food, compared to those who didn't. Of the 133 foods tested, only breakfast cereal showed a significant difference.

**Question:** Why is the researcher's conclusion that eating breakfast cereal increases the chances of conceiving a boy inappropriate? Hint: think about how this situation is similar to the one in the [xkcd comic](https://xkcd.com/882/)?
