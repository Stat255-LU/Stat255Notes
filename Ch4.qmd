...
```{r}
IC_Model <- lm(data=Icecream1, amount~time)
IC_Model
```

The intercept and slope are $b_0=$ `r IC_Model$coef[1] |> round(2)` and $b_1=$ `r IC_Model$coef[2]|>round(2)`.
...
The estimates $b_0$ and $b_1$ are close, but not identical to the true values of $\beta_0$ and $\beta_1$, which in this case, we know are $\beta_0=0$ and $\beta_1=2$. Typically, the values of $b_0$ and $b_1$ calculated from a sample will deviate from the true values $\beta_0$ and $\beta_1$ due to noise in the observed data.
...
A **t-distribution** is a symmetric, bell-shaped curve. Its shape is similar to that of the normal distribution, only it has more area in the tails, and is less concentrated around the center. When the sampling distribution for a statistic is symmetric and bell-shaped, it can be approximated by a t-distribution.
...
Just as the ratio of a regression statistic to its standard error follows a t-distribution, F-statistics also follow a known probability distribution when the assumptions of the ordinary linear regression model are appropriate.
...
For an F-distribution with 2 and 20 degrees of freedom, the probability of getting an F-statistic more extreme than 1.8 is about 0.19, while the probability of getting an F-statistic as extreme or more than 4.6 is about 0.02.
...
In this section, we'll apply the theories and formulas from the previous section to the models on the Florida lakes data that we saw in Chapter 3. We'll illustrate how to calculate all of the important pieces of information provided in the model summary output produced by R.
...
- The **Multiple R-Squared** value is the $R^2$ value seen in Chapter 2. $R^2 = \frac{\text{SST} -\text{SSR}}{\text{SST}}$
...
```{r}
gf_dist("f", df1=1, df2=51, geom = "area", fill = ~ (x < 25.24), show.legend=FALSE) + geom_vline(xintercept=25.24, color="red")  + xlab("F") + theme_bw() + xlim(c(0,30)) 
```
...
**Alternative Hypothesis:** The average mercury level among all Florida lakes with pH = 0 is not 0. ($\beta_0 \neq 0$).
...
$t=\frac{{b_j}}{\text{SE}(b_j)} = \frac{1.53}{0.203} = 7.72$

In our sample, the mean mercury level in lakes with pH 0 is estimated to be 7 standard errors higher than we would expect if the mean mercury level in lakes with pH 0 was 0. This is not a meaningful interpretation since no lakes have pH 0.
...
$SE(b_0)=\frac{s}{\sqrt{n_\text{Deep}}} = \frac{0.337}{\sqrt{15}} \approx 0.087$
...
The plot shows the estimated regression line from our sample (in blue), along with "true" regression line in read (which we wouldn't know in a real situation). The shaded gray area represents a 95% confidence interval for the average amount of ice cream dispensed for a given amount of time the dispenser is held.
...
We are 95% confident that an individual who holds the dispenser for 3 seconds will get between `r round(predict(IC_Model, newdata=data.frame(time=3), interval = "predict", level=0.95)[2],2)` and `r round(predict(IC_Model, newdata=data.frame(time=3), interval = "predict", level=0.95)[3],2)` oz of ice cream.
...
Trainees were praised after performing well and criticized after performing badly. The flight instructors observed that "high praise for good execution of complex maneuvers typically results in a decrement in performance on the next try."
...
IcecreamD <- data.frame(time, amount) # set up data table
...
Write the null hypothesis associated with the p-value on the line `write`. What (if anything) should we conclude from this p-value?
