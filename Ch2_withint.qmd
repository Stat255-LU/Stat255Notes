...
```{r}
Houses <- home_prices |> mutate(price=exp(price),
                                waterfront = ifelse(waterfront==1, "Yes", "No"),
                                ID = row_number() )|>
                          select(ID, price, sqft_living, waterfront, condition)
...
PM3 <- ggplot(data=Houses, aes(x=sqft_living, y=price, color=waterfront)) + 
  geom_point() + 
  theme_bw() + 
...
Residplot_M0 <-ggplot(data=Houses, aes(x = 1:nrow(Houses), y = price)) +geom_point() + 
  ...
  geom_abline(slope=0, intercept=mean(M0_House$fitted.values)) + xlab("")  + theme_bw() + theme(axis.text.x = element_blank()) + ylim(c(0,1000))
...
Residplot_sqft <- ggplot(data=Houses, aes(x = sqft_living, y = price)) +  geom_point() +
                ...
                stat_smooth(method="lm", se=FALSE) + ylim(c(0,1000)) +
                theme_bw() 
```
...
M1aResid <-ggplot(data=Houses_NWF, aes(x = 1:nrow(Houses_NWF), y = price)) +geom_point() + geom_segment(aes(xend = 1:nrow(Houses_NWF), yend = MNWF$fitted.values), color="red") + 
  geom_abline(slope=0, intercept=mean(MNWF$fitted.values)) + xlab("")  + theme_bw() + theme(axis.text.x = element_blank()) + ylim(c(0,1000)) + ggtitle("NonWaterfront")
M1bResid <-ggplot(data=Houses_WF, aes(x = 1:nrow(Houses_WF), y = price)) +geom_point() + geom_segment(aes(xend = 1:nrow(Houses_WF), yend = MWF$fitted.values), color="red") + 
  geom_abline(slope=0, intercept=mean(MWF$fitted.values)) + xlab("")  + theme_bw() + theme(axis.text.x = element_blank())+ ylim(c(0,1000)) + ylab("") + theme(axis.text.y = element_blank()) + ggtitle("Waterfront")
```
...
Residplot_MR <- ggplot(data=Houses, aes(x=sqft_living, y=price, color=waterfront)) + geom_point()+ geom_parallel_slopes(se=FALSE) + 
...
```{r}
ggplot(data=Houses, aes(x=sqft_living, y=price, color=waterfront)) + geom_point()+stat_smooth(method="lm", se=FALSE) + ylim(c(0,1000)) + theme_bw()
```
...
```{r}
((SST - SSR_cond)/(2-0))/(SSR_cond/(200-(2+1)))
```
...
p1 <- ggplot(data=df1, aes(y=y, x=Group))+ geom_point() + ggtitle("Scenario 1") + theme_bw()
p2 <- ggplot(data=df2, aes(y=y, x=Group))+ geom_point() + ggtitle("Scenario 2") + theme_bw()
```
...
**1.\**  We'll work with the data set on 146 roller coasters from around the world. We'll attempt to predict the speed of a coaster (in mph).   
...
b) Speeds are broken down by type of coaster (steel or wooden).  
...
**2.\** Continuing with the roller coaster dataset, we'll now predict the speed of the coaster (in mph) using its design and duration (in seconds) as explanatory variables.    

There are several coaster designs, most of which have only a few coasters. 

```{r}
summary(Coasters$Design)
```
We see that there are a lot of sit down coasters and then a few of each of the other types. We'll modify the variable to categorize design as either "Sit Down" or "Other". 
...
A scatterplot displaying the relationship between duration, design, and height is shown below.    

```{r}
ggplot(data=Coasters, aes(x=Duration, y=Speed, color=Design))+
  geom_point() + 
...
Notice that there are 2 coasters with very fast speeds, of 100 mph or more. Let's remove these outliers from the dataset since they seem to be drastically different than the other coasters, and fit the model to the filtered data. 
...
```{r}
ggplot(data=Coasters_filtered, aes(x=Duration, y=Speed, color=Design))+
  geom_point() + 
  xlab("Duration in Seconds") + 
  ylab("Top Speed in MPH") + 
  geom_parallel_slopes(se=FALSE) + 
...
```{r}
M1 <- lm(data=Coasters_filtered, Speed ~ Duration + Design)
M1
```

a) Write sentences interpreting the estimates $b_0$, $b_1$, and $b_2$ (i.e. the numbers under `Intercept`, Duration`, and `DesignSit Down`). State the numerical value of each estimate specifically in your interpretation, as is done in Section 2.1 of the class notes.  



b) Use the model to calculate the predicted speed of a sit down coaster with duration 150 seconds.      

c) Is the prediction you calculated in (b) consistent with the scatterplot? How certain can you be about the prediction? Give a range of speeds you think the coaster could plausibly lie in.     

d) Suppose that Coasters A and B are both sit down coasters and that Coaster A has a duration 10 seconds longer than B. Which coaster has the faster predicted speed, and by how much?   

e) Suppose that Coasters C and D have the same duration, and that Coaster A is a sit down coaster, and B isn't. Which coaster has the faster predicted speed, and by how much?   



**3.\** Continuing with the model in Questions 2 and 3 (excluding outliers), the $R^2$ value is shown below. 

```{r}
summary(M1)$r.squared
```

a) Write a sentence interpreting this value in context.   

b) We add year built (since 1900) as an explanatory variable. The $R^2$ value is shown below.      

```{r}
M2 <- lm(data=Coasters_filtered, Speed ~  Duration + Design + Year_Since1900)

summary(M2)$r.squared
```

Does adding year built appear to help predict the height of a coaster? Explain your answer.   

c) Going back to the model M in Question 2, we now drop design from the model and use only duration as an explanatory variable. The $R^2$ value is shown below.  

```{r}
M2b <- lm(data=Coasters_filtered, Speed ~ Duration)
summary(M2b)$r.squared
```

Does dropping design from the model hurt our ability to predict height? Explain your answer.   


**4.\** The scatterplot shows the relationship between top speed of a coaster (in mph) and its maximum height. We again exclude the two outlier coasters.   
...
```{r}
M4 <- lm(data=Coasters_filtered, Speed ~  Max_Height + Design)
M4
```

a) Would you expect the model that predicts height using top speed as an explanatory variable to have a higher or lower $R^2$ value than a model that uses duration as an explanatory variable (hint: compare this scatterplot to the one in Question 2)?   

b) R output for to the model using top speed is shown below.
...
**5. \** The estimates $b_0$, $b_1$ and $b_2$ for the model M4 in Question 4 are shown below. 
...
Suppose that instead of using these, we used the model used the model:

$$
\widehat{\text{Speed}} = 25 + 0.25\times\text{Max_Height} + 3\times{\text{DesignSitDown}}
$$ 

Can we say how SST, SSR, and $R^2$ for this new model would compare to Model 4? If so, what can we say and why?    
...
a) For the fictional test score example in 6(b), write the expression that would need to be minimized to obtain the values of regression coefficients $b_0$ and $b_1$. Substitute values from the data into your expression. (Hint: The only variables in the expression should be $b_0$ and $b_1$).   

b) There are three different ways to do this part of the question. You may complete any of the three ways. To get the most from the question, it is recommended that you complete the part that is consistent with the amount of mathematics you have taken previously.    
...
**8.\** The plots show three different lines fit to the same sets of points. 
...
Thus, the predicted value for Group A is $b_0$ and for Group B is $b_0 + b_1$. 

a) Let $b_0 = 5$ and $b_1 = 2$ (so that the predicted value for each group corresponds to the mean response value for that group). **Calculate SSR.**  

b) Suppose instead that we let $b_0 = 4$ and $b_1 = 2$ (so that the predicted value for each group corresponds to the median response value for that group). **How would you expect SSR for this model to compare to the one determined by the means? Calculate SSR to verify your answer.**   

**10. \** Consider the following four models. Note the model names M0-M4 might be different than in the previous questions. Recall that there are a total of 117 coasters used in the dataset, after excluding missing values and outliers.            

```{r}
M0 <- lm(data=Coasters_filtered, Speed ~ 1)
M1 <- lm(data=Coasters_filtered, Speed ~ Duration)
M2 <- lm(data=Coasters_filtered, Speed ~ Duration + Design)
M3 <- lm(data=Coasters_filtered, Speed ~ Max_Height + Design)
```


Residual sum of squares (SSR) are shown for each model. Calculate F-statistics to compare the following pairs of models. 
...
Are there any pairs of models that we cannot calculate an F-statistic to compare? Explain your answer. 


**11. \** 

a) Continuing with the models for roller coaster speeds in Question 10, the F-statistic for comparing Model 0 to Model 2 is shown below. Based on the size of this F-statistic, what can we conclude about the variable(s) in these models?    
...
b) 
The F-statistic for comparing Model 1 to Model 2 is shown below. Based on the size of this F-statistic, what can we conclude about the variable(s) in these models?    
...
**12. \** The illustrations below show three different scenarios, each with a categorical explanatory variable with three levels. The red dots indicate the mean for each group.     
...
**13.\ ** The scatterplot shows a response variable (y), plotted against a quantitative explanatory variable (x), with a categorical explanatory variable, (either A or B) indicated by color and shape. 
...
a) In which model will the estimate of $b_1$ be greater? Justify your answer.   

b) Consider Models 1 and 2 and also the following model:
