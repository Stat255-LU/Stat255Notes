...
4.  Independence: The response value for each observation is not affected by any of the other observations (except due to explanatory variables included in the model).
...
$\text{Price}_i = \beta_0 + \beta_1\text{Waterfront}_{i}+ \beta_2\text{Sq.Ft.}_{i} + \epsilon_i$, where $\epsilon_i\sim\mathcal{N}(0,\sigma)$.
...
```{r}
resid_panel(model = M_House_price_wf_sqft, smoother=TRUE)
```
...
When there are violations of model assumptions, we can sometimes correct for these by modeling a function of the response variable, rather than the response variable itself. When the histogram of residuals and normal qq plot show signs of right-skewness, modeling a logarithm of the response variable is often helpful.

### Example: Modeling Car Prices

We'll work with data on a set of 110 new cars, released in 2020. The data come from the `Lock5Data` package in R.
...
When using the `predict` command, R gives predictions for log(Price), so we need to exponentiate.
...
$$
\begin{aligned}
\text{log(Salary)}_i & = \beta_0 + \beta_1\times\text{Year}_i + \beta_2\times\text{Age}_i + \beta_3\times\text{Age}^2_i + \\
& + \beta_4\times\text{HSGrad}_i + \beta_5\times\text{SomeColl}_i + \beta_6\times\text{CollegeGrad}_i  \\
& + \beta_7\times\text{AdvDeg}_i + \beta_8\times\text{JobInf}_i  \\
& + \epsilon_i,  \\
\text{where} \epsilon_i\sim\mathcal{N}(0, \sigma)
\end{aligned}
$$
...
```{r, fig.height=8, fig.width=8}
resid_xpanel(M_logWage_price_yr_age2_ed_job, smoother=TRUE)
```
...
- For each year since 1993, average salary is expected to multiply by `r exp(M_logWage_price_yr_age2_ed_job$coefficients)[2] |>round(3)`, a `r ((exp(M_logWage_price_yr_age2_ed_job$coefficients)[2] -1) |>round(3))*100` percent increase, assuming all other variables are held constant. Since the p-value on `year` is large, we cannot conclusively say that salaries are changing over time.

- Workers with a high school degree are estimated to make `r ((exp(M_logWage_price_yr_age2_ed_job$coefficients)[5] -1) |>round(3))*100` percent more than workers with less than a high school degree, assuming all other variables are held constant.

- Workers with some college are estimated to make `r ((exp(M_logWage_price_yr_age2_ed_job$coefficients)[6] -1) |>round(3))*100` percent more than workers with less than a high school degree, assuming all other variables are held constant.

- Workers with a college degree are estimated to make `r ((exp(M_logWage_price_yr_age2_ed_job$coefficients)[7] -1) |>round(3))*100` percent more than workers with less than a high school degree, assuming all other variables are held constant.

- Workers with an advanced degree are estimated to make `r ((exp(M_logWage_price_yr_age2_ed_job$coefficients)[8] -1) |>round(3))*100` percent more than workers with less than a high school degree, assuming all other variables are held constant.

The differences in education levels are all statistically discernible.

- Workers with an informational job are estimated to make `r ((exp(M_logWage_price_yr_age2_ed_job$coefficients)[9] -1) |>round(3))*100` percent more than workers with an industrial one, assuming all other variables are held constant. This difference is not big enough to be statistically discernible.
...
We see that this expression has a negative quadratic coefficient, meaning it will have the shape of a downward facing parabola. A sketch of the equation $$y=`r (summary(M_logWage_price_yr_age2_ed_job)$coefficients[4]) |> round(5)` \times \text{Age}^2 + `r (summary(M_logWage_price_yr_age2_ed_job)$coefficients[3]) |> round(5)` \times \text{Age}$$ is shown below.
...
$$
\widehat{Price} = b_0 + b_1\times\text{Waterfront} + b_2\times\text{SqFt}
$$

Recall that this model assumes the slope relating price and square footage is the same ($b_2$) for houses on the waterfront as for houses not on the waterfront. An illustration of the model is shown below.
...
$$
\widehat{Price} = b_0 + b_1\times\text{Waterfront} + b_2\times\text{Sq. Ft.} + b_3\times\text{Waterfront}\times\text{Sq.Ft}
$$
...
$$
\begin{aligned}
\widehat{Price} & = b_0 + b_1\times\text{1} + b_2\times\text{Sq. Ft.} + b_3\times\text{1}\times\text{Sq.Ft} \\
& = (b_0+b_1) + (b_2+b_3)\times{\text{Sq. Ft.}}
\end{aligned}
$$ For a house not on the waterfront ($\text{waterfront}=0$), the equation relating price to square feet is

$$
\begin{aligned}
\widehat{Price} & = b_0 + b_1\times\text{0} + b_2\times\text{Sq. Ft.} + b_3\times\text{0}\times\text{Sq.Ft} \\
& = b_0 + b_2\times{\text{Sq. Ft}}
\end{aligned}
$$

The intercept is $b_0$ for non-waterfront houses, and $b_0 + b_1$ for waterfront houses.

The slope is $b_2$ for non-waterfront houses, and $b_2 + b_3$ for waterfront houses.
...
```{r}
M_House_price_wf_sqft_int <- lm(data=Houses, price~waterfront*sqft_living)
summary(M_House_price_wf_sqft_int)
```

```{r, echo=FALSE}
House_Int_b0 <- M_House_price_wf_sqft_int$coef[1] |>round(2)
House_Int_b1 <- M_House_price_wf_sqft_int$coef[2] |>round(2)
House_Int_b2 <- M_House_price_wf_sqft_int$coef[3] |>round(2)
House_Int_b3 <- M_House_price_wf_sqft_int$coef[4] |>round(2)
```

The regression equation is

$$
\widehat{Price} = `r House_Int_b0` + `r House_Int_b1`\times\text{Waterfront} + `r House_Int_b2`\times\text{Sq. Ft.}  + `r House_Int_b3`\times\text{Waterfront}\times\text{Sq.Ft}
$$
...
$$
\begin{aligned}
\widehat{Price} & = `r House_Int_b0` + `r House_Int_b1`\times\text{1} + `r House_Int_b2`\times\text{Sq. Ft.} + `r House_Int_b3`\times\text{1}\times\text{Sq.Ft} \\
& = (`r House_Int_b0` + `r House_Int_b1`) + (`r House_Int_b2`+`r House_Int_b3`)\times{\text{Sq. Ft.}} \\
& = `r House_Int_b0 + House_Int_b1` + `r House_Int_b2 + House_Int_b3`\times{\text{Sq. Ft.}}
\end{aligned}
$$
...
$$
\begin{aligned}
\widehat{Price} & = `r House_Int_b0` + `r House_Int_b1`\times\text{0} + `r House_Int_b2`\times\text{Sq. Ft.} + `r House_Int_b3`\times\text{0}\times\text{Sq.Ft} \\
& = `r House_Int_b0` + `r House_Int_b2`\times{\text{Sq. Ft.}} 
\end{aligned}
$$ **Interpretation**

When interpreting $b_0$ and $b_2$, we need to state that the interpretations apply only to the "baseline" category (in this case non-waterfront houses).
...
- $b_1$ - On average, the price of a waterfront house with 0 square feet is expected to be `r House_Int_b1` thousand dollars less than the price of a non-waterfront house with 0 square feet. This is not a sensible interpretation in this case.

- $b_2$ - For each additional square foot in size, the price of a non-waterfront house is expected to increase by `r House_Int_b2` thousand dollars.
...
Alternatively, we could interpret $b_0+b_1$ and $b_2+b_3$ together.

- $b_0 + b_1$ - On average, a house with 0 square feet that is on the waterfront is expected to cost `r House_Int_b0 +House_Int_b1` thousand dollars. This is not a sensible interpretation since there are no houses with 0 square feet.

- $b_2 + b_3$ - For each additional square foot in size, the price of a waterfront house is expected to increase by `r House_Int_b2 +House_Int_b3` thousand dollars.
...
$$
\widehat{Price}_1 =  `r House_Int_b0` + `r House_Int_b1`\times0 + `r House_Int_b2`\times `r Houses$sqft_living[1]`  + `r House_Int_b3`\times0\times `r Houses$sqft_living[1]` = `r (House_Int_b0 + House_Int_b2*Houses$sqft_living[1]) |> round(0)` \text{ thousand dollars}
$$

$$
\widehat{Price}_2 =  `r House_Int_b0` + `r House_Int_b1`\times1 + `r House_Int_b2`\times `r Houses$sqft_living[1]`  + `r House_Int_b3`\times `r Houses$sqft_living[1]` \times 1 = `r House_Int_b0 + House_Int_b1 + (House_Int_b2 + House_Int_b3)*Houses$sqft_living[1] |> round(0)` \text{ thousand dollars}
$$
...
```{r}
summary(M_House_price_wf_sqft_int)$r.squared
```

**Model without Interaction**

```{r}
summary(M_House_price_wf_sqft)$r.squared
```

We see that adding an interaction term improved the proportion of variability in house price explained by the model from `r summary(M_House_price_wf_sqft)$r.squared |> round(2)` to `r summary(M_House_price_wf_sqft_int)$r.squared |> round(2)`. This is a fairly notable increase.
...
```{r}
anova(M_House_price_wf_sqft, M_House_price_wf_sqft_int)
```
...
$$
\widehat{LogPrice} = b_0 + b_1\times\text{Acc060} 
$$
...
P2 <- ggplot(data=Cars2020, aes(x=Weight, y=log(Price))) + geom_point() + stat_smooth(method="lm", se=FALSE) + theme_bw() 
...
We see that highway MPG is negatively associated with price, weight is positively associated with price, and that there is also a negative relationship between weight and highway MPG.
...
We've learned, however, that when something seems unusual, we should think deeper. We noticed that highway gas mileage was negatively associated with weight, and that both are negatively associated with price. Since heaviest cars tend to also be the most expensive, it may be that cars that get better gas mileage are less expensive because they are smaller and weigh less.
...
Let's add weight to the model and see what happens to estimate associated with highway MPG.
...
We've already included information about size, through the weight variable, so we won't include it too. Let's add drive type to the model.
...
The intercept theoretically tells us that a car that accelerates from 0 to 60 mph in no time, gets 0 mpg, weighs 0 lbs, and is AWD would be expected to cost `r exp(M_Cars6$coefficients)[1] |> round(4)` thousand dollars. This statement is nonsensical and we shouldn't try to interpret the intercept.

The price of a car is expected to decrease by `r ((1-exp(M_Cars6$coefficients[2]))*100) |> round(0)` percent for each additional second it takes to accelerate from 0 to 60 mph, assuming highway MPG, weight and type of drive are held constant.

The price of a car is expected to increase by `r ((exp(M_Cars6$coefficients[3])-1)*100) |> round(1)` percent for each additional highway MPG, assuming acceleration time, weight and type of drive are held constant.

The price of a car is expected to increase by `r ((exp(M_Cars6$coefficients[4])-1)*100) |> round(3)` percent for each additional pound in weight, assuming acceleration time, highway MPG, and type of drive are held constant. This is equivalent to a `r ((exp(M_Cars6$coefficients[4])-1)*100*1000) |> round(0)` percent increase for each additional 1,000 pounds in weight.
...
We are 95% confident that the average price of all new 2020 cars that weigh 3000 lbs, take 8 seconds to accelerate from 0 to 60 mph, and have all wheel drive will cost between `r (exp(predict(M_Cars6, newdata=newcar, interval="confidence", level=0.95))[2]) |> round(0)` and `r (exp(predict(M_Cars6, newdata=newcar, interval="confidence", level=0.95))[3]) |> round(0)` thousand dollars.
...
We are 95% confident that the price of an individual car that weighs 3000 lbs, takes 8 seconds to accelerate from 0 to 60 mph, and has all wheel drive will cost between `r (exp(predict(M_Cars6, newdata=newcar, interval="prediction", level=0.95))[2]) |> round(0)` and `r (exp(predict(M_Cars6, newdata=newcar, interval="prediction", level=0.95))[3]) |> round(0)` thousand dollars.
...
library(Lock5Data)
data("HomesForSale2e")
HomesForSale <- HomesForSale2e |> mutate(State=as.factor(State))
```
...
The model summary output is displayed below.
...
Continuing with the house price dataset in Questions 1-6, we add information about the number of bedrooms and bathrooms, in addition to size and state, to the model to predict log(price).
