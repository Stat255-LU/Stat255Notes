...
ggplot(data=Sampdf, aes(y=y, x=x))+geom_point() + theme_bw()
```
...
P0 <- ggplot(data=Sampdf, aes(y=y, x=x))+geom_point() + geom_hline(yintercept = mean(Sampdf$y), color="blue") + theme_bw() + ggtitle("Constant Model - Degree 0")

P1 <- ggplot(data=Sampdf, aes(y=y, x=x))+geom_point() + stat_smooth(method="lm", se=FALSE, color="blue")  + theme_bw() + ggtitle("Linear Model - Degree 1")

P2 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 2, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Quadratic Model - Degree 2")

P3 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 3, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Cubic Model - Degree 3")

P4 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 4, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Quartic Model - Degree 4")

P5 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 5, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Degree 5 Polynomial Model")

P6 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 6, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Degree 6 Polynomial Model")

P7 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 7, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Degree 7 Polynomial Model")

P8 <- ggplot(data=Sampdf, aes(y=y, x=x)) + geom_point() + stat_smooth(method="lm", se=TRUE, fill=NA, formula=y ~ poly(x, 8, raw=TRUE),colour="blue")  + theme_bw() + ggtitle("Degree 8 Polynomial Model")

grid.arrange(P0, P1, P2, P3, P4, P5, P6, P7, P8)
```

The plot below shows models of degree 0, 1, 3, and 7 fit to the training data on the same plot. 

```{r, echo=FALSE, fig.height=5, fig.width=8}
ggplot(data = Sampdf, aes(x=x, y=y)) + 
  geom_point() +
  stat_smooth(aes(color = "Degree 0"), method='lm', formula = y~1, se=FALSE) +
  stat_smooth(aes(color = "Degree 1"), method='lm', formula = y~poly(x,1), se=FALSE) +
  stat_smooth(aes(color = "Degree 3"), method='lm', formula = y~poly(x,3), se=FALSE) +
  stat_smooth(aes(color = "Degree 7"), method='lm', formula = y~poly(x,7), se=FALSE) +
  scale_color_manual(name = "Polynomial Models",
                     values = c("Degree 0" = "blue",
                                "Degree 1" = "darkgreen",
                                "Degree 3" = "purple",
                                "Degree 7" = "red")) +
  theme_minimal() +
...
```{r, fig.height=5, fig.width=8, echo=FALSE}
ggplot(data=Newdf, aes(y=y, x=x))+geom_point() + theme_bw()
```
...
We plot models 0, 1, 3, and 7 and see which best fits the data. 
...
predictions <- data.frame(
  x = x_seq,
  M0 = predict(Sim_M0, newdata = new_data),
  M1 = predict(Sim_M1, newdata = new_data),
 # M2 = predict(Sim_M2, newdata = new_data),
  M3 = predict(Sim_M3, newdata = new_data),
#  M4 = predict(Sim_M4, newdata = new_data),
#  M5 = predict(Sim_M5, newdata = new_data),
  M7 = predict(Sim_M7, newdata = new_data)
)

# Reshape data for plotting
predictions_long <- predictions %>%
  pivot_longer(cols = -x, names_to = "Model", values_to = "Predicted")

# Plot test data with all model curves
ggplot() +
  geom_point(data = Newdf, aes(x = x, y = y), alpha = 0.6, size = 2) +
  geom_line(data = predictions_long, aes(x = x, y = Predicted, color = Model), 
            size = 1) +
  scale_color_manual(values = c("M0" = "blue", "M1" = "darkgreen",
                                 "M3" = "purple",  "M7" = "red"),
                     labels = c("M0 (degree 0)", "M1 (degree 1)",
                                "M3 (degree 3)",  "M7 (degree 7)")) +
  labs(title = "New Data with Fitted Polynomial Models",
       x = "x",
       y = "y") +
  theme_minimal() +
  theme(legend.position = "right")
...
```{r, echo=FALSE}
Degree <- 0:8
RMSPE <- c(RMSPE0, RMSPE1, RMSPE2, RMSPE3, RMSPE4, RMSPE5, RMSPE6, RMSPE7, RMSPE8) |> round(2)
kable(data.frame(Degree, RMSPE), caption="Prediction error on new data")
```
...
```{r, echo=FALSE}
Degree <- 0:8
RMSE <- c(RMSE0, RMSE1, RMSE2, RMSE3, RMSE4, RMSE5, RMSE6, RMSE7, RMSE8) |> round(2)
kable(data.frame(Degree, RMSE), caption="Error on training data")
```
...
```{r, echo=FALSE}
RMSPEdf <- RMSPEdf %>% pivot_longer(c(Test_Error, Train_Error), names_to="Error_Type", values_to = "Error")
```

```{r, echo=FALSE}
ggplot(data=RMSPEdf, aes(x=Degree, y=Error, color=Error_Type)) +  geom_point() + ylim(c(2.7,4.5)) + geom_line() + theme_bw()
```
...
```{r, echo=FALSE}
equation1 <- function(x){4.5*x -1.4*x^2 +0.1*x^3}
...
PM3 <- ggplot(data=df, aes(y=y, x=x, color=samp, size=samp)) + geom_point() + 
        stat_function(fun=equation1,geom="line",color="yellow", size=3) +
        stat_function(fun=equation2,geom="line",color="blue", size=3) + theme_bw()
PM3
```
...
1.  **Model Bias** - Our model might not be complex enough to capture the true relationship between the response and explanatory variable(s). For example, if we use a linear or quadratic model when the true relationship is cubic, our predictions will suffer from model bias. Model bias pertains to the difference between the true response function value $f(x^*)$, and the expected value of $\hat{f}(x^*)$ that would be obtained in the long run over many samples.

2.  **Model Variance** - Individual observations in the training data are subject to random sampling variability. As model complexity increases, it becomes more flexible and prone to being pulled away from the true relationship because of outliers in the training data.

3.  **Prediction Variance** - Even if two observations have the same value(s) for every explanatory variable, they will usually have different response values, due to random noise (i.e. the $\epsilon_i\sim\mathcal{N}(0,\sigma)$ term). So, even if we really knew the relationship between the response and explanatory variable(s), we would still not get every prediction exactly correct.
...
```{r, echo=FALSE, fig.height=6, fig.width=10, cache=FALSE}
x <- seq(from=1, to=3, by=0.01)
Var <- x^2/10
Bias <- 1/x
Error <- sqrt(Bias^2+Var) 
...
CVRMSE <- Housing_ridge$results$RMSE
ggplot(data=data.frame(lambda, CVRMSE), aes(x=lambda, y=CVRMSE))+geom_line() + xlim(c(0,2)) + ylim(c(0.4, 0.5)) + ggtitle("Ridge Regression Cross Validation Results") + theme_bw()
```
...
library(rpart)
library(rpart.plot)
tree <- rpart(SalePrice~., data=Ames_Train, cp=0.001)
rpart.plot(tree, box.palette="RdBu", shadow.col="gray", nn=TRUE, cex=1, extra=1)
```
