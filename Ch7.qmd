...
36. Explain how model complexity relates to training and test error, prediction variance and bias, and overfitting. \
37. Explain how to use cross-validation in model selection. \
38. Explain how complexity parameters associated with ridge regression, decision trees, and splines impact variance, bias, and likelihood of overfitting. \
39. Make predictions from a decision tree. \
40. Calculate classification accuracy, sensitivity, specificity, confusion matrix, and receiver operating characteristic curves.\
41. Assess ethical considerations associated with predictive models in context.
...
We use Model 4 to predict the prices of the new houses. The first 1- predictions are shown below. 

```{r}
predictions <- predict(model4, newdata=Ames_Test)  # substitute your best model
head(data.frame(predictions), 10)
```

A quick look at the data reveals that the houses with the highest predicted prices, among these ten, were the newest ones, each built in 2007.
