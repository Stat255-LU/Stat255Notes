...
**Research Question: How much do trees grow in a decade?**

Barro Colorado Island, located in the man-made Gatun Lake in the middle of the Panama Canal is home to the world's most heavily researched tropical rainforest. Every individual tree in the 50 hectare forest was permanently numbered with an aluminum tag in 1982 and researchers have visited every tree repeatedly since them, recording measurements on each individual tree. You can read more about the Barro Colorado Island forest study in [this article](https://smithsonian.figshare.com/articles/dataset/_Dataset_Barro_Colorado_Forest_Census_Plot_Data_Version_2012_/25620873) by Condit, Lao, Perez, Dolins, & Hubbell (2012). The data referenced in this section come from this study.

We want to explore how much tree diameters in the forest change in a decade. Researchers visited all 9,189 trees that were in the forest in 2014 and still alive in 2025. They recorded the change in diameter in cm. for each tree. The distribution of changes in tree diameter is shown below.

```{r}
BCI_Forest <- readRDS("trees.RDS")
BCI_Forest <- BCI_Forest |> filter(census %in% c(2014, 2025)) |>
  pivot_wider(
    id_cols = unique.id,
    names_from = census,
    values_from = -c(unique.id, census),
    names_glue = "{.value}_{census}"
  ) |> filter(status_2014 == "alive" & status_2025 == "alive" & !is.na(dbh_2025)& !is.na(dbh_2014)) |> 
  transmute(unique.id = unique.id, 
            plot = plot_2025,                                           
            sp6 = sp6_2025,
            diameter = dbh_2025/10, #convert mm to cm
            growth = dbh_2025-dbh_2014,
...
```{r}
BCI_Forest_Plot_Pop <- ggplot(data=BCI_Forest, aes(x=growth)) + 
  geom_histogram(fill="blue", binwidth=10, color="white") + 
  ggtitle("Change in Tree Diameter in cm") + theme_bw() + xlim(c(-500, 500))
BCI_Forest_Plot_Pop
```

```{r}
BCI_Forest |> summarize(Mean_Growth = mean(growth) |> round(1),
                        Median_Growth = median(growth) |> round(1) ,
                        SD_Growth = sd(growth) |> round(1), 
                        N = n())
```
...
In this case, we'll denote the intercept $\beta_0$, rather than $b_0$, as we've done before. It is customary to use Greek letters to denote population parameters, and English letters to denote sample statistics. Since we are fitting the model to all 9,189 trees, we'll call the intercept $\beta_0$.

```{r}
M_BCI_growth <- lm(data=BCI_Forest, growth ~ 1)
M_BCI_growth
```

```{r, echo=FALSE}
M_BCI_growth_b0 <- M_BCI_growth$coef[1] |> round(2)
```

On average, tree diameter increased by about $\beta_0 =$ `r M_BCI_growth_b0` cm. between 2014 and 2025.

### Population and Sample

The Barro Colorado Island Forest data are unusual in that researchers actually visited and recorded data on every tree in the sector of the forest. Typically, we do not have the time, money, or means to collect data on all individuals or items of interest. Instead, we collect data on only a subset of the individuals, and then use information from those observed to draw conclusions about the broader set we're interested in.

**Vocabulary:**

1.  A **population** is the entire set of individuals that we want to draw conclusions about.
2.  A **census** is a study in which the entire population is observed.\
3.  A **sample** is a subset of a population.\
4.  A **parameter** is a numerical quantity pertaining to an entire population or process.\
5.  A **statistic** is a numerical quantity calculated from a sample.

In the Barro Island study, the population of interest is all trees in the 50-hectare section of the forest. Because researchers actually visited all 9,189 trees, this study is an example of a census.

Parameters and statistics are numeric quantities (such a mean, median, proportions, etc.) that describe the population or sample, respectively. In this study, a parameter of interest is the mean growth among all trees. Because we have data on all 9,189 trees, we know that the population mean is `r mean(BCI_Forest$growth) |> round(1)` cm. In most situations, we won't know the exact value of a population parameter since we will only have data on a sample, rather than on the entire population.

In this section, we'll explore how to use statistics computed from samples to draw conclusions about population parameters.

### Sampling Distribution and Standard Error

Suppose that researchers did not have the time to visit all 9,189 trees in the forest and instead had selected a random sample of 100 trees, recording the change in diameter of just those 100.

**First Sample**

We use the `sample` command to take a random sample of 100 trees. The first 6 residents in the sample are shown below.
...
```{r}
# take sample of 100 trees
Sample1 <- slice_sample(BCI_Forest, n = 100)
```

The distribution of diameter growth for the 100 trees in the sample is shown below.

```{r}
BCI_Plot_Sample1 <- ggplot(data=Sample1, aes(x=growth)) + 
  geom_histogram(fill="blue", binwidth=10, color="white") + 
  ggtitle("Change in Tree Diameter in cm") + theme_bw() + xlim(c(-500, 500))
BCI_Plot_Sample1
```
...
```{r}
Sample1 |> summarize(Mean_growth = mean(growth) |> round(2),
                        SD_growth = sd(growth) |> round(2) , 
                        N= n())
```

Notice that the mean growth in the sample differs from that of the population. This is not surprising, as we wouldn't expect a statistic calculated from a sample of 100 to exactly match the parameter calculated from the entire population.

We'll fit a model to the sample data and record the intercept.

```{r}
M_Sample1_growth <- lm(data=Sample1, growth ~ 1)
M_Sample1_growth
```

In this case, we'll denote the intercept $b_0$, since it is a statistic calculated from our sample. $b_0$ = `r M_Sample1_growth$coefficients |>round(2)` is an estimate of our population mean $\beta_0$, which we happen to know is `r M_BCI_growth_b0`
...
To investigate that, let's take some additional samples of size 100, fit a model to estimate the mean growth $b_0$ and see how much $b_0$ varies between samples.

**Second Sample**

```{r}
Sample2 <- slice_sample(BCI_Forest, n = 100)
M_Sample2_growth <- lm(data=Sample2, growth ~ 1)
M_Sample2_growth
```
...
```{r}
Sample3 <- slice_sample(BCI_Forest, n = 100)
M_Sample3_growth <- lm(data=Sample3, growth ~ 1)
M_Sample3_growth
```
...
```{r}
Sample4 <- slice_sample(BCI_Forest, n = 100)
M_Sample4_growth <- lm(data=Sample4, growth ~ 1)
M_Sample4_growth
```
...
```{r}
Sample5 <- slice_sample(BCI_Forest, n = 100)
M_Sample5_growth <- lm(data=Sample5, growth ~ 1)
M_Sample5_growth
```

We see that the mean growth varies between samples.

Let's now take 10,000 more random samples of 100 trees and record $b_0$, the record estimated mean diameter growth for each sample.

```{r}
Samples_growth <- mosaic::do(10000)*lm(growth  ~ 1, data = slice_sample(BCI_Forest, n=100))
```

The histogram below shows the distribution of $b_0$, the mean tree growth in the 10,000 different samples.

```{r}
Samp_Dist_Mean_growth <- ggplot(data=Samples_growth, aes(x=Intercept)) +
  geom_histogram(color="white", fill="lightblue", binwidth=1) + 
  ggtitle("Sampling Distribution for b0 - Mean Diameter Growth") + 
  xlab("Mean Growth (in cm.)") + theme_bw()
Samp_Dist_Mean_growth
```

We notice that most of our 10,000 samples yielded means between 10 and 40 cm. and that the distribution is roughly symmetric and bell-shaped.

The distribution shown in this histogram is called the **sampling distribution for** $b_0$. Since $b_0$ represents the mean tree diameter growth, in this case we can call it the sampling distribution for the mean.The **sampling distribution** for a statistic shows the distribution of the statistic over many samples.
...
```{r}
Samples_growth |> summarize(Mean_b0 = mean(Intercept) |> round(1),
                              SE_b0 = sd(Intercept)|> round(1), 
                              N= n())
```

Notice that the mean of the sampling distribution for $b_0$ is about the same as the population mean $\beta_0$ = `r M_BCI_growth_b0`

The standard deviation of the sampling distribution for the mean $b_0$, however, is much less than the population standard deviation of `r sd(BCI_Forest$growth) |> round(0)`. This makes sense, since there is much less variability in the sampling distribution for the mean growth than there was in the distribution of growth for individual trees. We saw that some trees grew more than 100 cm. in diameter, while others actually got smaller. On the other hand, the mean growth in samples of 100 typically stayed between 10 and 40 cm.

The standard deviation of the sampling distribution for a statistic is also called the **standard error** of the statistic. In this case, it represents the amount of variability in mean diameter growth between different samples of 100 trees. This is different than the population standard deviation, which represents the amount of variability in growth between individual trees.
...
Suppose the sample consisted of 10, or 50, or 200 trees, instead of 100? Would you expect the standard deviation of individual tree growth in the samples to increase, decrease or stay about the same? What about the standard error of the mean growth?

The histogram shows the distribution of sample standard deviations in random samples of each size. These represent the variability in growth between individual trees in the sample.

```{r, echo=FALSE}
set.seed(08202026)
Sample10 <- slice_sample(BCI_Forest, n = 10)
Sample50 <- slice_sample(BCI_Forest, n = 50)
Sample100 <- slice_sample(BCI_Forest, n = 100)
Sample200 <- slice_sample(BCI_Forest, n = 200)
```
...
Hist10 <- ggplot(data = Sample10, aes(x=growth)) + 
  geom_histogram(color="white", fill="lightblue", binwidth=10) + 
  ggtitle("n=10") + 
  xlab("Growth") + ylab("Frequency") + xlim(xrange) + theme_bw()

Hist50 <- ggplot(data = Sample50, aes(x=growth)) + 
  geom_histogram(color="white", fill="lightblue", binwidth=10) + 
  ggtitle("n=50") + 
  xlab("Growth") + ylab("Frequency") + xlim(xrange) + theme_bw()


Hist100 <- ggplot(data = Sample100, aes(x=growth)) + 
  geom_histogram(color="white", fill="lightblue", binwidth=10) + 
  ggtitle("n=100") + 
  xlab("Growth") + ylab("Frequency") + xlim(xrange) + theme_bw()

Hist200 <- ggplot(data = Sample200, aes(x=growth)) + 
  geom_histogram(color="white", fill="lightblue", binwidth=10) + 
  ggtitle("n=200") + 
  xlab("Growth") + ylab("Frequency") + xlim(xrange) + theme_bw()

grid.arrange(Hist10, Hist50, Hist100, Hist200, ncol=2)
```

Notice that the variability in growth between individual trees stays roughly the same, regardless of sample size.

The table shows the standard deviation of individual tree growth in each of the samples.

```{r, echo=FALSE}
SD10 <- sd(Sample10$growth) |> round(1)
SD50 <- sd(Sample50$growth) |> round(1)
SD100 <- sd(Sample100$growth) |> round(1)
SD200 <- sd(Sample200$growth) |> round(1)
Sample_Size <- c(10,50, 100, 200)
SD <- c(SD10, SD50, SD100, SD200)
kable(data.frame(Sample_Size, SD))
```

Sample size does not impact the amount of variability between individuals in a sample. Standard deviation in individual tree growths does not systematically increase or decrease based on sample size (though it will vary randomly between samples). This makes sense, since variability between growths of individual trees has nothing to do with the size of the sample.

Now, we'll examine what happens to the standard error of the mean $b_0$ as the sample size changes.

```{r, echo=FALSE}
Mean10 <- mosaic::do(10000) * lm( growth ~ 1, data = slice_sample(BCI_Forest, n=10))
Mean50 <- mosaic::do(10000) * lm( growth ~ 1, data = slice_sample(BCI_Forest, n=50))
Mean100 <- mosaic::do(10000) * lm( growth ~ 1, data = slice_sample(BCI_Forest, n=100))
Mean200 <- mosaic::do(10000) * lm( growth ~ 1, data = slice_sample(BCI_Forest, n=200))
```
...
Mean50_Plot <- ggplot(data=Mean50, aes(x=Intercept)) +  
  geom_histogram(color="white", fill="lightblue", bins=60) +
  xlab("Sample Mean") + ylab("Frequency") +
  ggtitle("n=50") + xlim(c(-50,100)) + 
  theme(legend.position = "none")  + theme_bw()

Mean100_Plot <- ggplot(data=Mean100, aes(x=Intercept)) +  
  geom_histogram(color="white", fill="lightblue", bins=100) +
  xlab("Sample Mean") + ylab("Frequency") +
  ggtitle("n=100") + xlim(c(-50,100)) + 
  theme(legend.position = "none")  + theme_bw()
...
grid.arrange(Mean10_Plot, Mean50_Plot, Mean100_Plot, Mean200_Plot, ncol=2)
```

Notice that as the sample size increases, the sampling distribution of the mean becomes more symmetric and bell-shaped, and also more concentrated around the population mean $\beta_0$.

The table shows the standard error of the mean for samples of different size:

```{r, echo=FALSE}
SE10 <- sd(Mean10$Intercept) |> round(1)
SE50 <- sd(Mean50$Intercept) |> round(1)
SE100 <- sd(Mean100$Intercept) |> round(1)
SE200 <- sd(Mean200$Intercept) |> round(1)
Sample_Size <- c(10,50,100,200)
SE <- c(SE10, SE50, SE100, SE200)
...
The sampling distribution for the mean diameter growth is shown again below. The population mean $\beta_0=$ `r mean(BCI_Forest$growth) |> round(1)` is marked by the purple dotted line. The gold bar at the bottom of the histogram represents the range of sample means that lie within $\pm 2$ standard errors of the true population mean:

```{r, echo=FALSE}
beta0 <- mean(BCI_Forest$growth)
b0 <- mean(Sample1$growth)
SE_b0 <- sd(Samples_growth$Intercept)
```

`r round(M_BCI_growth_b0,2)` - 2(`r round(SE_b0 ,2)`) = `r round(M_BCI_growth_b0 -2*SE_b0 ,2)` to \newline

`r round(M_BCI_growth_b0,2)` + 2(`r round(SE_b0 ,2)`) = `r round(M_BCI_growth_b0 +2*SE_b0 ,2)`.

```{r}
Samp_Dist_Mean_growth + geom_vline(xintercept=M_BCI_growth_b0, color="purple", linetype="dotted", linewidth=2) +
  geom_segment(aes(x= M_BCI_growth_b0 - 2*SE_b0, xend= M_BCI_growth_b0 + 2*SE_b0, y=20, yend=20), color="gold", size=20, alpha=0.01) 
```

We calculate the proportion of samples whose mean growth lies within $\pm 2$ standard errors of the true mean.

```{r}
Lower <- M_BCI_growth_b0 - 2*SE_b0 
Upper <- M_BCI_growth_b0 + 2*SE_b0 
sum((Samples_growth$Intercept >=Lower) & (Samples_growth$Intercept <= Upper))
```

Approximately 95% of the 10,000 samples produced proportions within $\pm 2$ standard errors of the true population mean.

In a real situation, we won't have access to the entire population, but only a single sample. For example, recall our original sample of 100 trees, in which we observed a mean diameter growth of $b_0=$ `r mean(Sample1$growth)|>round(2)`.

Since we now know that 95% of all samples produce sample means that lie within two standard errors of the population mean, we can obtain an estimate of the population mean $\beta_0$ by adding and subtracting $2\times \text{SE}(b_0)$ from our observed sample estimate $b_0$.
...
We'll calculate a 95% confidence interval for the mean diameter growth, using our original sample where $b_0$ = `r mean(Sample1$growth)|>round(2)`. The 95% confidence interval is:
...
Based on our sample of 100 trees, we can be 95% confident that the mean diameter growth among all trees in this section of the Barro Colorado Island forest from 2014 to 2025 was between `r round(b0 - 2*SE_b0, 2)` and `r round(b0 + 2*SE_b0, 2)` cm
...
Suppose, instead of $\beta_0=$ `r beta0`, our sample of 100 had resulted in a mean of $\beta_0=$ `r 35` cm? We can see from the sampling distribution that some samples did produce means that low.

Then, using the same formula we would obtain an approximate 95% confidence interval of

$$\begin{aligned}
& 30 \pm 2\times \text{SE}(b_0) \\
& = `r 35` \pm 2(`r round(SE_b0,2)`)
\end{aligned}$$

which is `r round(35 - 2*SE_b0, 2)` to `r round(35 + 2*SE_b0, 2)`.

Notice that this interval does not contain the true value of $\beta_0=$ `r beta0|>round(1)`. Does this mean we did something wrong when we calculated the confidence interval?
...
```{r}
Samples_df_b0 <- Samples_growth %>% mutate(Sample = row_number(),
                                                b_0 = Intercept,
                                                Lower = b_0 - 2*SE_b0, 
                                                Upper = b_0 + 2*SE_b0,
...
The picture shows confidence intervals produced by the first 100 samples, but we actually took 10,000 different samples of 100 trees. Let's calculate how many of these samples produced confidence intervals that contain the true value of $\beta_0$.

```{r}
mosaic::prop(Samples_df_b0$Contains_beta0 == TRUE)
```
...
**Research Question: Do Mercury levels in Florida lakes exceed environmental standards?**
...
The Florida Lakes data come from the `Lock5Data` R package.
...
Suppose we want to estimate the mean mercury level among all Florida lakes. As we saw in the previous section, we should not expect the population mean to exactly match the sample, due to random variability between samples. We can, however, use the sample statistic as an estimates and construct confidence intervals for the unknown population means.
...
```{r}
M_Lakes_merc_0_Boot3 <- lm(data=Bootstrap3, Mercury~1)
M_Lakes_merc_0_Boot3
```
...
**Conclusion:** Based on our sample of 53 lakes, we estimate that the mean mercury level among all Florida lakes is between about 0.4 and 0.6 ppm. This is safely below the US legal limit of 1 ppm, though right around the 0.5 ppm, which is Canada's legal limit. This interval is based on the assumption that our sample of 53 lakes is reasonably representative of all lakes in Florida.
...
**Research Question: How do Mercury mercury levels in Lakes in Northern Florida compare to lakes in Southern Florida?**
...
A confidence interval for $b_1$ will tell us a range of plausible values for the difference in mean mercury level among all lakes in Southern Florida, compared to Northern Florida.
...
The bootstrap estimates are $b_0$ = `r M_Lakes_merc_loc_Boot2$coef[1]` and $b_1$ = `r M_Lakes_merc_loc_Boot2$coef[2]`.
...
This tells us how much the mean mercury level among lakes in Northern Florida would vary between different samples of 33 Northern lakes.
...
This tells us how much the difference in mean mercury level would be expected to vary between samples of 20 Southern Florida Lakes and 33 Northern Florida lakes.

A 95% confidence interval for the difference in mean mercury level between all Southern and Northern Florida lakes is given by
...
We are 95% confident that the mean mercury level among all lakes in Southern Florida is between `r round(CI$lower[2],2)` and `r round(CI$upper[2],2)` higher than the mean mercury level among all lakes in Northern Florida.

Since the confidence interval for the difference does not include 0, we have evidence that mercury levels are higher, on average, in Southern Florida than in Northern Florida. The size of the difference, however, is not very clear, given the width of the interval. A difference of 0.1 ppm is fairly small and perhaps not very meaningful, while a difference as big as 0.4 ppm would be a meaningful difference.

### CI for Regression Slope

**Research Question: Is there a relationship between mercury concentration and pH level in Florida Lakes?**
...
**Conclusion** We are 95% confident that for each one-unit increase in pH, mercury level is expected to decrease between about 0.1 and 0.19 ppm.

### CI for Regression Response

**Research Question: What is the expected mercury level in a lake in Southern Florida with a neutral pH level of 7?**
...
**Conclusion:** We are 95% confident that the mean mercury level among lakes in Southern Florida with pH=7 is between 0.51 and 0.74 ppm.
...
1.  A standard-error bootstrap interval is appropriate when the sampling distribution for our statistic is roughly symmetric and bell-shaped. When this is not true, a percentile bootstrap interval can be used as long as there are no gaps or breaks in the bootstrap distribution.

2.  In situations where there are gaps and breaks in the bootstrap distribution, then the bootstrap distribution may not be a reasonable approximation of the sampling distribution we are interested in.

3.  When working with a categorical variable, it is a good idea to resample in each group, with sizes matching those in the original samples. For example, in the Florida lakes study, rather than resampling 53 lakes, we would ideally resample 33 Northern lakes and 20 Southern lakes, matching the numbers from the original study. The `mosaic` package, which we used to do bootstrapping does not allow this option, unfortunately. In this case, the sample sizes are similar enough that this issue will not have a big impact on our results. If sample sizes within groups were small or very different, we may want to write our own code to do this.
...
**Research Question: Is there evidence of a difference in mean mercury level between lakes in Northern and Southern Florida?**
...
**Research Question: Is there evidence of a difference in mean chlorophyll between lakes in Northern and Southern Florida?**
...
We see that on average chlorophyll levels were higher among the lakes in northern Florida by about 4 mg/$m^3$. We'll use a permutation test to investigate whether such a difference could occur by random selection of lakes if there is really no relationship between chlorophyll and location.
...
The histogram shows the distribution of differences in the group means observed in our simulation.
...
Our p-value, representing the proportion of simulations resulting in difference in means more extreme than 3.9, is
...
**Conclusion:** Since the p-value is quite large (much bigger than common criteria like 0.05 or 0.10), we cannot rule out the null hypothesis. It would not be unusual to see data like we did in a situation where the null hypothesis is actually true. Thus, we do not have enough evidence to conclude that average chlorophyll amount differs between lakes in Northern and Southern Florida.
...
**Research Question: Is there evidence of a relationship between Mercury level and pH in Florida Lakes?**
...
**Research Question: Is there evidence of a difference in mean mercury level between shallow, medium, and deep lakes in Florida?**
...
**Alternative Hypothesis:** There is a difference in average mercury levels between shallow, medium, and deep lakes.
...
The F-statistic observed in the shuffled data is `r Lakes_merc_depth_perm1$F`.
...
**Conclusion:** Since the p-value is large, it is plausible that we would see differences like we did in the actual data in a situation where the null hypothesis is true. We cannot reject the null hypothesis. We do not have enough evidence to say that average mercury levels differ between shallow, medium, and deep lakes.
...
\end{aligned}$$ R output for the model is shown below.
...
\end{aligned}$$ where the Humanities, NaturalScience, and SocialScience variables all have value 1 if the course is in that field and 0 otherwise.
...
```{r}
FB_b0_b1_Bootstrap |> summarize(Mean = mean(X1) |> round(1), 
                                      SE=sd(X1) |> round(1))
```
...
Each day, United Airlines has a flight from Boston to San Francisco, departing at 6 am (Eastern time). The flight is scheduled to arrive in San Francisco at 9:30 am (Pacific time). This results in an expected total flight duration of 6.5 hours (390 minutes). Due to weather conditions, however, the exact duration of the flight varies from day to day. A random sample of flights from 28 different days was selected. The times of the 28 flights are shown below, listed from shortest to longest.

```{r, echo=FALSE}
library(Lock5Data)
data("Flight433_3e")
Flight433 <- Flight433_3e
Flight433 |> arrange(AirTime)
```
...
library(knitr)
library(kableExtra)
T <- data.frame(Flight433)  %>% dplyr::summarize(Mean=mean(AirTime)|> round(1), StDev=sd(AirTime)|> round(1), n=n())
kable(T)
```
...
\end{aligned}$$ Model estimates are shown below.
...
Interpret the p-value in context by completing the sentence "The p-value represents the probability of \_\_\_\_\_, assuming \_\_\_\_\_."
...
```{r}
prop(~ (F > 8.128), data = science_schtyp_perm_test)
```
...
Interpret p-value in context by completing the sentence "The p-value represents the probability of \_\_\_\_\_, assuming \_\_\_\_\_."
...
Interpret p-values in context by completing the sentence "The p-value represents the probability of \_\_\_\_\_, assuming \_\_\_\_\_."
...
The Bootstrap distribution for the difference in average cost of books in natural science classes, compared with humanities classes, was given in Question 5(c). If we perform a permutation-based hypothesis test for the null hypothesis that there is no difference in average books costs among natural science courses, compared to humanities courses, how would the distribution of mean differences compare to the bootstrap distribution from 5(c)? What would be different about it? Would the p-value associated with the hypothesis test be more or less than 0.05? Explain your answer.
